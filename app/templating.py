...
def plural(count: int, forms: str) -> str:
    ...
    две у остальных. Правило выбирается по текущему языку, а не по числу
    форм, потому что ноль во французском — ещё единственное число, а
    в английском, испанском и немецком уже множественное.
    """
    parts = forms.split("|")
    if len(parts) >= 3:

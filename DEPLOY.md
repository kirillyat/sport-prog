...
## 6а. Автовыкатка из Gitea

На сервере стоит таймер `sport-autodeploy.timer`: раз в минуту он спрашивает
у Gitea хеш ветки `main` и, если тот отличается от развёрнутого, забирает
изменения и пересобирает контейнер тем же скриптом, что и ручная выкатка.

...
| `DEFAULT_LANGUAGE` | язык портала для тех, кто ничего не выбрал: `ru`, `en` или `fr` | по умолчанию `ru`; выбор человека и подсказку браузера не перебивает |
| `COURSE_TITLE`, `COURSE_SUBTITLE` | заголовок раздела «Курс» | даёт владелец портала |
